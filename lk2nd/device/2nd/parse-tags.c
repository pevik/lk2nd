...
#define MIN_ATAG_SIZE	(sizeof(struct atag) / sizeof(uint32_t))
...
static bool parse_atags(const void *tags)
{
	...
	if (copy_size)
		atag_copy = copy = malloc(copy_size);

	for (t = tags; t->tag; t = next_atag(t)) {
		switch (t->tag) {
		case ATAG_CMDLINE:
			lk2nd_dev.cmdline = strndup(t->data, atag_data_size(t));
			break;
		}
		if (copy && copy_atag(t)) {
...
const void *lk2nd_device2nd_parse_tags(void)
{
	...
	/* Make sure the DTB/ATAGS are located fully in RAM */
	if (check_aboot_addr_range_overlap(addr, LK2ND_MAX_TAG_SIZE) ||
	    check_ddr_addr_range_bound(addr, LK2ND_MAX_TAG_SIZE)) {
		dprintf(CRITICAL, "Invalid DTB/ATAGS address: %p\n", tags);
		return NULL;
	}

	/* Make sure the DTB/ATAGS region is fully mapped */
	arm_mmu_map_region(addr, addr, LK2ND_MAX_TAG_SIZE,
